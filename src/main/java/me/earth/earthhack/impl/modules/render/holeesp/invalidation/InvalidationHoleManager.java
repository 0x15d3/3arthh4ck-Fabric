...
public class InvalidationHoleManager extends SubscriberImpl implements Globals, HoleManager
{
    ...
    public InvalidationHoleManager(InvalidationConfig config)
    {
        ...
        listeners.add(new ReceiveListener<>(BlockUpdateS2CPacket.class,
        ...
        listeners.add(new LambdaListener<>(BlockStateChangeEvent.class, event ->
        {
            if (config.isUsingInvalidationHoleManager())
            {
                ...
                IChunk chunk = event.chunk();
                chunk.earthhack$addHoleTask(() ->
                {
        ...
        {
            ((IChunk) event.chunk()).earthhack$setHoleVersion(((IChunk) event.chunk()).earthhack$getHoleVersion() + 1);
            if (config.isUsingInvalidationHoleManager() && mc.world != null)
            {
                holes.entrySet().removeIf(e ->
                {
                    if (!mc.world.isChunkLoaded(e.getKey()))
                    {
        ...
        listeners.add(new ReceiveListener<>(ChunkDataS2CPacket.class, e -> e.addPostEvent(() ->
        {
            if (config.isUsingInvalidationHoleManager())
            {
                ChunkDataS2CPacket p = e.getPacket();
                Chunk chunk = mc.world.getChunk(p.getChunkX(), p.getChunkZ());
                HoleFinder compiler = new HoleFinder(chunk, config.getHeight(), this);
                ((IChunk) chunk).earthhack$setCompilingHoles(true);
                ((IChunk) chunk).earthhack$setHoleVersion(((IChunk) chunk).earthhack$getHoleVersion() + 1);
    ...
    private void addPostCompilationTask(BlockPos pos, BlockState state, IChunk chunk)
    {
        chunk.earthhack$addHoleTask(() ->
        {
